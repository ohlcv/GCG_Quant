...
# ================== 配置区域 ==================
CONFIG = {
    "target_path": r"C:\Users\Excoldinwarm\Desktop\GCG_Quant",
...
class CodeAnalyzer:
    ...
    def _is_comment(self, line: str) -> bool:
        ...
        if self.in_comment_block:
            if (
                self.config.get("multi_comment")
                and self.config["multi_comment"][1] in line
            ):
                self.in_comment_block = False
            return True

        if self.config.get("single_comment") and line.startswith(
            self.config["single_comment"]
        ):
            return True

        if self.config.get("multi_comment") and line.startswith(
            self.config["multi_comment"][0]
        ):
